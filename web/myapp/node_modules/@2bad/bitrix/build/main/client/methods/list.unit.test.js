...
describe('Client `fillWithCommands` method', () => {
    it('should fill array with required amount of commands to process all entries', () => {
        const command = { method: methods_1.Method.CRM_DEAL_LIST, params: { select: ['*'] } };
        const start = 0;
        const toProcess = 7;
        const entriesPerCommand = 2;
        expect(list_1.fillWithCommands(command, start, toProcess, entriesPerCommand)).toMatchSnapshot();
    });
    ...
    it('should override `params.start`', () => {
        const wrongStart = 99;
        const command = { method: methods_1.Method.CRM_DEAL_LIST, params: { start: wrongStart } };
        const start = 0;
        const toProcess = 2;
        const entriesPerCommand = 1;
        const commands = list_1.fillWithCommands(command, start, toProcess, entriesPerCommand);
...
describe('Client `highest` method', () => {
    it('should get highest value from object', () => {
        expect(list_1.highest({ a: undefined })).toBe(undefined);
        expect(list_1.highest({ a: undefined, b: undefined })).toBe(undefined);
        expect(list_1.highest({ a: 1, b: undefined })).toBe(1);
        expect(list_1.highest({ a: undefined, b: 1 })).toBe(1);
        expect(list_1.highest({ a: 1, b: 2 })).toBe(2);
        expect(list_1.highest({ a: 2, b: 1 })).toBe(2);
    });
    it('should get highest value from array', () => {
        expect(list_1.highest([undefined])).toBe(undefined);
        expect(list_1.highest([undefined, undefined])).toBe(undefined);
        expect(list_1.highest([1, undefined])).toBe(1);
        expect(list_1.highest([undefined, 1])).toBe(1);
        expect(list_1.highest([1, 2])).toBe(2);
        expect(list_1.highest([2, 1])).toBe(2);
    });
});
describe('Client `batchToListPayload` method', () => {
    it('should convert batch with named payloads to a list payload', () => {
        ...
        expect(list_1.batchToListPayload(payload)).toMatchSnapshot();
    });
    ...
    it('should handle undefined in `result`', () => {
        const payload = {
            result: {
                result: [undefined, [{ ID: '1' }], undefined],
                result_error: [],
                result_total: [],
                result_next: [],
                result_time: [{
...
describe('Client `list` method', () => {
    it('should make one request when entries can be fetched in one go', async () => {
        const callMock = jest.fn(() => Promise.resolve({ next: false }));
        const batchMock = jest.fn(() => Promise.resolve({}));
        const list = list_1.default({ call: callMock, batch: batchMock });
        await list(methods_1.Method.CRM_DEAL_LIST, {});
    ...
    it('should make multiple requests when entries can not be fetched in one go', async () => {
        const callMock = jest.fn(() => Promise.resolve({ next: 2, total: 120 }));
        const batchMock = jest.fn(() => Promise.resolve({
            result: {
                result: ['done'],
                result_error: [],
    ...
    it('should return a first request payload when entities can be fetched in a single request', async () => {
        ...
        const callMock = jest.fn(() => Promise.resolve(mockPayload));
        const batchMock = jest.fn(() => Promise.resolve({}));
        const list = list_1.default({ call: callMock, batch: batchMock });
        const payload = await list(methods_1.Method.CRM_DEAL_LIST, {});
    ...
    it('should return a flatten payload when entities can not be fetched in a single request', async () => {
        ...
        const callMock = jest.fn(() => Promise.resolve({ next: 2 }));
        const batchMock = jest.fn(() => Promise.resolve(mockPayload));
        const list = list_1.default({ call: callMock, batch: batchMock });
        const payload = await list(methods_1.Method.CRM_DEAL_LIST, {});
        expect(payload).toMatchSnapshot();
    });
    it('should default start to 0', async () => {
        const callMock = jest.fn(() => Promise.resolve({ next: 2, total: 20 }));
        const batchMock = jest.fn(() => Promise.resolve({
            result: {
                result: ['done'],
                result_error: [],
    ...
    it('should properly take into account start', async () => {
        ...
        const list = list_1.default({ call: callMock, batch: batchMock });
        await list(methods_1.Method.CRM_DEAL_LIST, { start: 27 });
        expect(callMock.mock.calls).toMatchSnapshot();
        expect(batchMock.mock.calls).toMatchSnapshot();
    });
...
//# sourceMappingURL=list.unit.test.js.map